        ...
        <div className="space-y-3">
          {data.map((issue, idx) => {
            ...
            return (
              ...
              >
                <div
                  className={`h-8 w-8 rounded-lg ${config.bg} ${config.darkBg} flex items-center justify-center shrink-0 mt-0.5`}
                >
                  <Icon
                    className={`h-4 w-4 ${config.color} ${config.darkColor}`}
                  />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-foreground leading-snug dark:text-gray-200">
                    {issue.title}
                  </p>
